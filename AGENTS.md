...
3. Para geometria de conexão, mexa em `connectionPoints.ts` / `connectorPath.ts` (pura) e reflita no drag em `DiagramCanvas.vue`. Self-loops (`from === to`, ex. `referred_by`): geometria fixa via `selfLoopPoints` (top@0.75 → right@0.25) + rota externa `selfLoopCorners`; Curved → `roundedPolylinePath`, Orthogonal → `polylinePath` (cantos 90°) — independente da notação. Barker Curved usa `splitRoundedPolyline` (divide o path já filetado no meio do comprimento, para o canto da junção sólido/pontilhado também arredondar). Handles escondidos, label fixa no ápice (sem drag — evita texto preso atrás do card). Markers `-start` reutilizam a geometria dos `-end` (`auto-start-reverse` já gira 180° — espelhar de novo desenharia o glifo para dentro do card). `loadState` descarta `labelPositions`/`connectorPoints`/`routeOverrides` de self-loops ao carregar (auto-cura de valores obsoletos). Mid-route: `routeOverrides[relId].orthogonal.midOffset` / `.curved.{along,bulge}` (frações assinadas, por estilo — trocar Curved↔Orthogonal preserva ambos); handle no meio do conector (hover), dblclick reseta o estilo actual. Curved: arrastar o miolo em 2 eixos (ao longo da corda + perpendicular), sempre entre as pontas com margem. Orthogonal: só HH/VV (dois cotovelos) com espaço útil; HV/VH de um cotovelo não editam o meio; o canal fica sempre entre as pontas com margem (~28px) para os markers. Mudar a ponta para outra aresta (soltar o drag) limpa os overrides daquela relação.
